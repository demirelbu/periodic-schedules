...
from functions import costfunction, allschedules
...
if __name__ == "__main__":
    ...
    for period in range(3, 13):  # period length
        ...
        sequences = allschedules(variables['no_plants'], period)
        # number of root to leaf branches (i.e., schedules)
        n = sequences.shape[0]
        # create a vector, including the costs associated with schedules
        cost_vector = np.zeros((n,))
        # compute the costs associated with schedules
        for index in range(n):
            cost_vector[index] = QuadraticCost(sequences[index, :])
        # print the period, the lowest cost, the corresponding schedule
        print("Period: {}, Min cost: {:3.4f}, Sequence: {}.".format(
            period, min(cost_vector), sequences[np.argmin(cost_vector), :]))
